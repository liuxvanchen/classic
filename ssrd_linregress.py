...
annual_ssrd_change_wm2 = np.diff(ssrd_annual_avg_wm2, axis=0)

mean_annual_ssrd_change_wm2 = np.mean(annual_ssrd_change_wm2, axis=(1, 2))

years = np.arange(1983, 1983 + mean_annual_ssrd_change_wm2.shape[0])

slope, intercept, r_value, p_value, std_err = linregress(years, mean_annual_ssrd_change_wm2)
...
plt.scatter(years, mean_annual_ssrd_change_wm2, label='实际年度辐射变化')
